...
\begin{titlepage}
  ...
  {\fontsize{25}{32}\selectfont\bfseries 自指、对角化与形式系统的边界}\par
  \vspace{0.7cm}
  {\Large 罗素悖论、哥德尔不完备性与图灵不可判定性}\par
  \vspace{1.0cm}
  {\large\color{Slate}一份从直觉到技术骨架的讲义}\par

  \vfill
  \begin{keybox}[阅读目标]
  三个结果看似分属集合论、证明论和计算理论，却共享一种结构：系统把关于自身的描述变成内部对象，再让这个对象作用于自身，并在关键位置实行反转。本讲义先解释这条共同主线，再分别重构图灵与哥德尔的思想。
  \end{keybox}
  \vspace*{1.3cm}
\end{titlepage}
...
这里没有得到“某件事既不能为真也不能为假”。罗素的矛盾说明假设的集合不能存在；哥德尔的“不可判定”首先是相对于理论的\textbf{不可证明与不可反驳}；一台具体机器是否停机也有确定答案，只是没有一种算法能统一给出所有答案。
\end{warnbox}
...
\node[box,right=of b] (c) {在自我作用处\textbf{取反或唱反调}};
\draw[arr] (a)--(b);
\draw[arr] (b)--(c);
\end{tikzpicture}
\end{center}
...
\begin{keybox}[记忆公式]
\centering
\Large 编号 $+$ 自用 $+$ 反转 $=$ 对角化的核心动力
\end{keybox}

\section{图灵机：先定义“机械计算”}

图灵面对希尔伯特的判定问题，首先必须回答：所谓“算法”究竟是什么？他的办法不是研究某一台现实机器，而是观察一个严格按规则计算的人，并删去一切与计算能力无关的细节。
...
\subsection{通用机：程序也可以成为数据}

每台图灵机都是一张有限规则表，因此可以编码为自然数 $\code{M}$。图灵构造出一台通用机 $U$：
\[
 U(\code{M},x)\quad\text{模拟}\quad M(x).
\]
同一台机器只要换一段编码，就能模拟不同机器。这是现代通用计算机的逻辑原型：\textbf{硬件保持不变，程序作为数据被读取和执行。}

\begin{warnbox}[两个“万能”必须区分]
\textbf{通用模拟机 $U$ 确实存在：}它能模拟任何已有算法。\\
\textbf{万能判定器 $H$ 不存在：}它不能预先判断任意程序在任意输入上会不会停机。\\
“能执行所有算法”不等于“能回答关于所有算法的一切问题”。
\end{warnbox}
...
\[
 \text{“这句话在理论 $T$ 中不可证明。”}
\]
这句话在自然语言里不难理解，困难在于：自然语言中的“这句话”依靠语境指向自己，而形式系统中没有这样含糊的指示语。因此，必须用一种科学、机械、无歧义的方法，让这个句子真正指向自身。
...
\begin{keybox}[真正的要求]
这个“代指”不能只靠我们从自然语言中体会出来，而必须有一套明确的构造规则，使理论能够机械地识别它究竟指向哪个句子。
\end{keybox}
...
\section{为什么必须自指？为什么选择“不可证明”？}

哥德尔要否定的是：一个一致而有效的理论能够判定所有算术句子。为此，他需要一个句子，使理论一旦证明它，就会反过来破坏自身的一致性。

普通句子“另一个句子 $A$ 不可证明”并不能做到这一点。即使它被证明了，也未必反过来约束它自己。只有让句子谈论自己的可证明性，证明行为才会反馈到句子的内容上。

\begin{center}
\begin{tikzpicture}[
  node distance=8mm and 10mm,
  every node/.style={font=\small,align=center},
  box/.style={draw=BlueLine,fill=PaleBlue,rounded corners=1mm,minimum height=10mm,text width=3.4cm},
  arr/.style={-{Latex[length=2.2mm]},draw=Slate,thick}
]
\node[box] (p) {$T$ 证明 $G$};
\node[box,right=of p] (q) {于是确实存在\ $G$ 的证明};
\node[box,below=of q] (r) {但 $G$ 的内容是“\,不存在这种证明”};
\node[box,left=of r] (s) {$T$ 得到冲突};
\draw[arr] (p)--(q);
\draw[arr] (q)--(r);
\draw[arr] (r)--(s);
\end{tikzpicture}
...
哥德尔没有使用“这句话是假的”，而是使用“这句话不可证明”，原因在于：

\[
 \text{不可证明}\neq\text{为假}.
\]

“这句话是假的”会形成直接的说谎者矛盾；“这句话不可证明”则允许一种稳定情况：句子为真，但理论无法证明它。哥德尔把会爆炸的真假悖论，转化成了揭示证明边界的不完备性结果。
...
\begin{center}
\color{Slate}\small
本讲义以直觉重构为主；哥德尔编码、对角引理的可表示性证明以及可证明性条件属于下一层技术细节。
\end{center}
